...
export namespace LLM {
  ...
  export async function stream(input: StreamInput) {
    ...
    if (process.env.OPENCODE_DEBUG_PROMPT === "1") {
      try {
        const dumpPath = path.join(os.tmpdir(), `opencode-prompt-${input.sessionID}.json`)
        const dump = {
          sessionID: input.sessionID,
          model: input.model,
          agent: input.agent,
        ...
        l.info("debug prompt dump", { file: dumpPath })
      } catch (error) {
        l.warn("debug prompt dump failed", { error: error instanceof Error ? error.message : String(error) })
      }
    }
